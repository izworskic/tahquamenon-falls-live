...
const plannerLines = js.split('\\n').map(line => line.trim());
assert(!plannerLines.some(line => line.startsWith(\"$('\") && line.includes('.forEach(')), 'Planner cannot call forEach on single-element $() selectors');
assert(!plannerLines.some(line => line.startsWith('$$$(')), 'Planner cannot reference an undefined $$$ selector');
assert(js.includes(\"$$('#timeChoices button').forEach\"), 'Time buttons must bind through the multi-element selector');
assert(js.includes(\"$$('#preferenceChoices button').forEach\"), 'Priority buttons must bind through the multi-element selector');
assert(js.includes(\"$$('[data-place-id]').forEach\"), 'Map buttons must bind through the multi-element selector');
